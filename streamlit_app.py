...
<style>
    ...
    .kpi-card:hover { transform: translateY(-3px); border-color: rgba(167,139,250,0.4); }
...
if page == "🏠 Overview":
    if df is None:
        ...
        for col, label, value, tone, sub in cards:
            with col:
                st.markdown(f"""
                <div class="kpi-card">
        ...
        if mode == "Paste feature values":
            ...
            if df is not None:
                if st.button("🎲 Draw random transaction"):
                    row = df.drop(columns=["Class"]).sample(1, random_state=None).iloc[0]
